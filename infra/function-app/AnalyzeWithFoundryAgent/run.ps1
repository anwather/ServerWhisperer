...
function Get-FoundryToken {
    try {
        return (Get-AzAccessToken -ResourceUrl 'https://ai.azure.com' -ErrorAction Stop).Token
    } catch {
        throw "Failed to acquire Foundry token. $($_.Exception.Message)"
    }
}
...
function Invoke-FoundryRequest {
    ...
    $uri = "${foundryEndpoint}${Path}${separator}api-version=${apiVersion}"
    $headers = @{
...
$thread = Invoke-FoundryRequest -Method 'POST' -Path "/threads" -Body @{}
$threadId = $thread.id
...
Invoke-FoundryRequest -Method 'POST' -Path "/threads/$threadId/messages" -Body @{
    role    = 'user'
    content = $messageContent
} | Out-Null

$run = Invoke-FoundryRequest -Method 'POST' -Path "/threads/$threadId/runs" -Body @{
...
while ($attempt -lt $maxAttempts) {
    ...
    $attempt++
    $runStatus = Invoke-FoundryRequest -Method 'GET' -Path "/threads/$threadId/runs/$runId" -Body $null
    ...
    if ($runStatus.status -eq 'requires_action') {
        ...
        Invoke-FoundryRequest -Method 'POST' -Path "/threads/$threadId/runs/$runId/submit_tool_outputs" -Body @{
...
$messages = Invoke-FoundryRequest -Method 'GET' -Path "/threads/$threadId/messages?order=desc&limit=1" -Body $null
